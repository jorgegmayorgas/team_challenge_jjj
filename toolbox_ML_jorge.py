...
def _internal_onehotencoder(df:pd.DataFrame,features_cat:list):
    """
...
def _internal_sns_pairplot(num_pair_plots:int,columns_per_plot:int,columns_for_pairplot:list,df:pd.DataFrame,target_col:str):
...
def get_features_num_classification (df:pd.DataFrame,target_col:str,pvalue:float=0.05):
    ...
    #ANOVA
    selected_columns=[]
    selected_columns= _internal_anova_columns(df,target_col,lista_num_columnas,pvalue)
    '''
    for columna in lista_num_columnas:
        unique_values=df[columna].unique()
        for valor in unique_values:
...
def plot_features_num_classification (df:pd.DataFrame,target_col:str="",columns:list=[],pvalue:float=0.5):
    ...
    """
    #ANOVA
    selected_columns= _internal_anova_columns(df,target_col,columns,pvalue)
    ...
    plt=_internal_sns_pairplot(num_pair_plots,columns_per_plot,columns_for_pairplot,df,target_col)
    '''
    for i in range(num_pair_plots):
        start_idx = i * columns_per_plot
        end_idx = (i + 1) * columns_per_plot
...
def plot_features_cat_classification(df:pd.DataFrame, target_col:str="", columns:list=[], mi_threshold:float=0.0, normalize:bool=False):
    ...
    if not 0.0 <= mi_threshold <= 1.0 and normalize:
        print("Error: 'mi_threshold' debe estar entre 0.0 y 1.0 cuando 'normalize' es True.")
        return
    ...
    selected_columns= _internal_anova_columns(df,target_col,categorical_cols,mi_threshold)
    columns_for_pairplot = df[selected_columns].columns
    columns_per_plot = 5
    # Calculate the number of pair plots needed
    num_pair_plots = len(columns_for_pairplot) // columns_per_plot
    plt=_internal_sns_pairplot(num_pair_plots,columns_per_plot,columns_for_pairplot,df)
...
def get_features_cat_classification(df:pd.DataFrame, target_col:str, mi_threshold:float=0.0, normalize:bool=False,relative:bool=False):
    ...
    * En caso de que "normalize" sea True:
    ...
    '''
    #FALTA es decir debe ser una variable categórica o numérica discreta pero con baja cardinalidad
    df_cardinalidad=describe_df(df)
    #pendiente mirar dentro de df_cardinalidad
    ...
    if normalize==False:
        ...
        for columna in df.columns:
            if columna!=target_col:
                    ...
                    if mi_score_categorical>=mi_threshold:
                        if columna not in selected_columns:
                            selected_columns.append(columna)
        print(mi_score_categorical,mutual_info_classification(df.drop(target_col,axis=1), df[target_col]))
    else:
        ...
        for columna in df.columns:
            if columna!=target_col:
                mi_normalized=mutual_info_classification(df[[columna]], df[target_col])/sum(list_mi_score_categorical)
                if mi_normalized>=mi_threshold:
                    selected_columns.append(columna)
            
        #mi_normalized = mutual_info_score(X, Y, contingency=None) / (2 * np.log(2))
        
        '''duda
        contra que comparo mi_normalized con el mutual information individual de cada columna y si es mayor o igual ese valor individual lo devuelvo?
        from sklearn.metrics import mutual_info_score

        # Assuming X and Y are arrays containing the values of the two variables
        mi_normalized = mutual_info_score(X, Y, contingency=None) / (2 * np.log(2))
        '''
...
def super_selector(df:pd.DataFrame,target_col:str,selectores):
    """
    ...
    * Si target_col no está vacío y es un columna válidad del dataframe, la función comprobará el valor de "selectores":
        ...
        * Si "selectores" no es un diccionario vacío, espera encontrar las siguientes posibles claves (y actúa en consecuencia):
            "KBest": Tendrá como valor el número de features a seleccionar aplicando un KBest. La función debe crear una lista con las features obtenidas de emplear un 
                    SelectKBest con ANOVA.  
            "FromModel": Tendrás como valores una lista con dos elementos, el primero la instancia de un modelo de referencia y el segundo un valor entero o compatible con 
                    el argumento "threshold" de SelectFromModel de sklearn. En este caso la función debe crear un a lista con las features obtenidas de aplicar un SelectFromModel con el modelo de referencia, y utilizando "threshold" con el valor del segundo elemento si este no es un entero. En este caso, cuando sea un entero, usarás SelectFromModel con los argumentos "max_features" igual al valor del segundo elemento y "threshold" igual a -np.inf. (Esto hace que se seleccionen "max_features" features)  
            "RFE": Tendrá como valor una tupla con tres elementos. El primero será un modelo instanciado, el segundo elemento determina el número de features a seleccionar 
                    y el tercero el step a aplicar. Serán los tres argumentos del RFE de sklearn que usará la función para generar una lista de features.
            "SFS": Tendrá como valor un tupla con 2 elementos, el modelo de referencia instanciado y el numero de featureas a alcanzar. Esta vez la feature empleará 
                    un SFS para obtener las lista de features seleccionadas.

    * La función debe devolver tantas listas seleccionadas como claves en el diccionario de selectores y una adicional con el resultado de aplicar un hard voting 
        a las listas obtenidas de aplicar el diccionario "selectores" y las que contenga "hard_voting", en caso de que "hard_voting" contenga una o más listas. 
        La función devolverá un diccionario con claves equivalentes a las de selectores pero con la lista correspondiente asignada a cada clave y 
        una adicional "hard_voting" caso de que "hard_voting" como argumento no sea una lista vacía.

    Argumentos:

        `df` (DataFrame): Variable dataframe de Pandas  sin valor por defecto.
        `target_col` (str): Variable target tipo str  sin valor por defecto.
        `selectores` (dict/None): Variable dict/None sin valor por defecto.
        
    
    Retorna:
        
        param_dict: Diccionario con lista de hiperparámetros
